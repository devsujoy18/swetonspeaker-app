<x-admin_layout>
	<x-admin_breadcrumb>
	    ...
	    <x-slot:breadcrumb_list>
	        <li class="breadcrumb-item active"> Keyfeatures</li>
	    </x-slot>
	</x-admin_breadcrumb>
	<section class="content">
	    ...
	    <x-admin_card>
	    	...
	    	<x-slot:card_tools>
	    		<a class="btn btn-primary btn-sm" href="{{ route('keyfeature.create')}}">
	    		...
	    		<table id="myTable" class="table table-bordered table-striped">
			        ...
			        <tbody>
			        	...
			        	<tr>
			        		...
			        		<td>{{ $keyfeature->order_no }}</td>
			        		<td>
			        			@if($keyfeature->status == 0) 
			        				Active 
			        			@else 
			        				Inactive 
			        			@endif
			        		...
			        		<td>
			        			<x-admin_editbtn>
			        				<x-slot:edit_link>
			        					{{ route('keyfeature.edit', $keyfeature->id) }}
			        				</x-slot>
			        			</x-admin_editbtn>

			        			@if($keyfeature->status == 0)
			        			<x-admin_changestatusbtn>
			        				<x-slot:status_link>{{ route('keyfeature.status', $keyfeature->id) }}</x-slot>
